	...
	success:function(data){
	    ...
	    if(status == 0 && Data){
                Data.forEach(function(d) {
                    ...
                    html += '		<i id="' + d.repoId + '" title="' + d.repoName + '" class="icon-write-down all-icon deal-method" onclick="fwrite(this)"></i>';
                    html += '	</th>';
                    html += '	<th class="th-4">--</th>';
                    html += '	<th class="th-5">'+ moment(d.modifyTime).format("YYYY-MM-DD HH:mm:ss") +'</th>';
                    html += '</tr>';
                });
