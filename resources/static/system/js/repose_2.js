	...
	$.ajax({
		...
		success:function(data){
			...
			if(status == 0 && result){
				...
				var file_html = '<a href="javascript:;" id="'+id+'" style="text-decoration:underline;" title="'+name+'" onclick="fshowDir(this)">'+name+'</a>';
...
function fshowDir(_this){
	...
	var text = $("#id-globle").children('a').text()+'/'+name;
	var html = '<a path="'+text+'" href="javascript:;" onclick="fshowDir(this)"><span style="padding:5px;">/</span>'+name+'</a>';
	$("#id-globle").children('a:last').append(html);
